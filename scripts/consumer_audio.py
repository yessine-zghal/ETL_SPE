...
def consumer_audio():
    ...
    for message in consumer:
        ...
        #     print(bytes(data[i]))
        #     data1.append(data[i])
        # # data1=base64.b64decode(message.value['data'])
        # # print(data1)
        # #bytes_wav = reduce(lambda x, y: x + y, map(lambda x: struct.pack('B', x), data))
        # print(data1)
        ...
        for i in tq(range(100),desc="Generating audio file.."):
            audio = AudioSegment.from_raw(bytes_io_wav, sample_width=2, frame_rate=22050, channels=1).export("/home/yessine/Desktop/Speech-Data-Pipeline/scripts/te.wav", format='wav')
            #.export('../audio/'+key+'.wav',format='wav')
